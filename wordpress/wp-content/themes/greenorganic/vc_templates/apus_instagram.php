		...
		<?php
		    ...
		    if ( $username != '' ) {
		        ...
		        if ( is_wp_error( $media_array ) ) {
		            ...
		            if ( $images_only = apply_filters( 'greenorganic_instagram_element_images_only', false ) ) {
		            ...
		            if ( $layout_type == 'grid' ) {
		        		...
		        		?>
		        		<div class="owl-carousel grid0" data-smallmedium="2" data-extrasmall="2" data-items="<?php echo esc_attr($columns); ?>" data-carousel="owl" data-pagination="false" data-nav="true">
			                <?php
			                foreach ( $media_array as $item ) {
			                    echo '<div class="item">';
			                    echo '<a href="'. esc_url( $item['link'] ) .'" target="'. esc_attr( $target ) .'"><img src="'. esc_url( $item[$size] ) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"/></a>';
