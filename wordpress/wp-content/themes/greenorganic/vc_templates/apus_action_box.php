...
if ( !empty($img) && isset($img[0]) ) $style = 'style="background:url('.esc_url_raw($img[0]).') repeat scroll 0 0 / 100% 100%"';
?>
    ...
    <?php if($style_box != 'style2'){ ?>
    	<div class="action-inner">
            ...
            <a class="btn btn-theme btn-outline" href="<?php echo esc_url_raw($link ); ?>">
                ...
                <?php }else{ ?>
                	<?php echo esc_html__('ORDER NOW','greenorganic') ?>
                <?php } ?>
            </a>
            <?php endif; ?>
        </div>
        ...
        <div class="action-v2 clearfix" <?php echo trim($bg_color); ?>>
            ...
            <?php if(!empty($image_box) && isset($image_box[0])){ ?>
                <div class="box-img col-xs-5">
                    <img src="<?php echo esc_url_raw($image_box[0]); ?>" alt="<?php esc_attr_e('Image', 'greenorganic'); ?>">
                </div>
            <?php }?>
