...
if ( !class_exists("Greenorganic_Woo_Custom") ) {
	class Greenorganic_Woo_Custom {

		public static function init() {
			// brand
			$tax = greenorganic_get_config( 'product_brand_attribute' );
			if ( !empty($tax) ) {
		...
		public static function orderby_list() {
			...
			if ( 1 != $wp_query->found_posts || woocommerce_products_will_display() ) {
				...
				$options = apply_filters( 'woocommerce_catalog_orderby', array(
					'menu_order'	=> esc_html__( 'Default', 'greenorganic' ),
					'popularity' 	=> esc_html__( 'Popularity', 'greenorganic' ),
					'rating'     	=> esc_html__( 'Average rating', 'greenorganic' ),
					'date'       	=> esc_html__( 'Newness', 'greenorganic' ),
					'price'      	=> esc_html__( 'Price: Low to High', 'greenorganic' ),
					'price-desc'	=> esc_html__( 'Price: High to Low', 'greenorganic' )
				) );
		...
		public static function add_brand() {
			...
			<div class="form-field">
				<label><?php esc_html_e( 'Thumbnail', 'greenorganic' ); ?></label>
		...
		public static function edit_brand( $term ) {
			...
			?>
			<tr class="form-field">
				<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'greenorganic' ); ?></label></th>
				<td>
		...
		public static function brand_image_field( $image = '' ) {
			...
			<div class="screenshot">
				<?php if ( $image ) { ?>
	                <img src="<?php echo esc_url($image); ?>" alt="<?php esc_attr_e('Image', 'greenorganic'); ?>"/>
	            <?php } ?>
			</div>
			...
			<div class="upload_image_action">
	            <input type="button" class="button add-image" value="<?php esc_html_e( 'Add', 'greenorganic' ); ?>">
	            <input type="button" class="button remove-image" value="<?php esc_html_e( 'Remove', 'greenorganic' ); ?>">
	        </div>
			<?php
		}
		...
		public static function brand_columns( $columns ) {
			...
			foreach ($columns as $key => $value) {
				if ( $key == 'name' ) {
					$new_columns['image'] = esc_html__( 'Image', 'greenorganic' );
				}
		...
		public static function brand_column( $columns, $column, $id ) {
			if ( $column == 'image' ) {
				...
				$columns .= '<img style="max-width: 60px;" src="' . esc_url( $image ) . '" alt="'.esc_html__( 'Image', 'greenorganic' ).'" class="wp-post-image" />';
			}
		...
		public static function get_product_brands() {
		    global $product;
		    $brands_tax = greenorganic_get_config( 'product_brand_attribute' );
	...
	add_action( 'init', array('Greenorganic_Woo_Custom', 'init') );
}