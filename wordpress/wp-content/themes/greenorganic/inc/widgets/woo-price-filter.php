...
class Greenorganic_Widget_Woo_Price_Filter extends WC_Widget {

	public function __construct() {
		$this->widget_cssclass    = 'apus_widget apus_widget_price_filter woocommerce widget_price_filter';
		$this->widget_description = esc_html__( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'greenorganic' );
		$this->widget_id          = 'apus_woocommerce_price_filter';
		$this->widget_name        = esc_html__( 'WooCommerce Price Filter List', 'greenorganic' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by price', 'greenorganic' ),
			...
			'price_range_size' => array(
				...
				'max'   => '',
				'std'   => 50,
				'label' => esc_html__( 'Price range size', 'greenorganic' )
			),
			'max_price_ranges' => array(
				...
				'max'   => '',
				'std'   => 10,
				'label' => esc_html__( 'Max price ranges', 'greenorganic' )
			),
			'hide_empty_ranges' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__( 'Hide empty price ranges', 'greenorganic' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( strlen( $min_price ) > 0 ) {
			$output .= '<li><a href="' . esc_url( $link ) . '">' . esc_html__( 'All', 'greenorganic' ) . '</a></li>';
		} else {
            $output .= '<li class="current">' . esc_html__( 'All', 'greenorganic' ) . '</li>';
		}
...
register_widget( 'Greenorganic_Widget_Woo_Price_Filter' );