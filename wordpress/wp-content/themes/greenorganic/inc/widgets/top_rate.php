...
if (class_exists('PostRatings')) {
    ...
    class Greenorganic_Top_Rate_Widget extends Apus_Widget {
        public function __construct() {
            parent::__construct(
                // Base ID of your widget
                'apus_top_rate_widget',
                // Widget name will appear in UI
                esc_html__('Apus Top Rate Widget', 'greenorganic'),
                // Widget description
                array( 'description' => esc_html__( 'The highest rated posts on your site', 'greenorganic' ), )
            );
        ...
        public function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php esc_html_e('Get most rated:', 'greenorganic'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('sort')); ?>"><?php esc_html_e('Sort by:', 'greenorganic'); ?></label>
                <select class="widefat" id="<?php echo esc_attr($this->get_field_id('sort')); ?>" name="<?php echo esc_attr($this->get_field_name('sort')); ?>">
                <option <?php selected('bayesian_rating', $instance['sort']); ?> value="bayesian_rating"><?php esc_html_e('Overall bayesian rating (score)', 'greenorganic'); ?></option>
                <option <?php selected('rating', $instance['sort']); ?> value="rating"><?php esc_html_e('Average rating', 'greenorganic'); ?></option>
                <option <?php selected('votes', $instance['sort']); ?> value="votes"><?php esc_html_e('Number of votes', 'greenorganic'); ?></option>
                </select>
            </p>
            ...
            <p>
                ...
                <?php esc_html_e('Descending', 'greenorganic'); ?>
                </label>
                ...
                <?php esc_html_e('Ascending', 'greenorganic'); ?>
                </label>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('date_limit')); ?>"><?php esc_html_e('Ignore posts older than:', 'greenorganic'); ?></label>
                <input id="<?php echo esc_attr($this->get_field_id('date_limit')); ?>" name="<?php echo esc_attr($this->get_field_name('date_limit')); ?>" type="text" value="<?php echo esc_attr( $instance['date_limit'] ); ?>" size="3" /> <?php esc_html_e('days', 'greenorganic'); ?>
                <br />
                <small><?php esc_html_e('(0 to ignore date)', 'greenorganic'); ?></small>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Limit:', 'greenorganic'); ?></label>
    ...
    register_widget( 'Greenorganic_Top_Rate_Widget' );
}