...
<form id="yith-wcwl-form" action="<?php echo trim($form_action); ?>" method="post" class="woocommerce">
        ...
        <div class="hidden wishlist-title <?php echo esc_attr( $is_custom_list ? 'wishlist-title-with-form' : '');?>">
            ...
            <?php if( $is_custom_list ): ?>
                <a class="btn button show-title-form">
                    ...
                    <?php esc_html_e( 'Edit title', 'greenorganic' ) ?>
                </a>
            <?php endif; ?>
        </div>
        <?php if( $is_custom_list ): ?>
	...
	<table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
        ...
        if( count( $wishlist_items ) > 0 ) :
            ...
            foreach( $wishlist_items as $item ) :
                ...
                if( $product && $product->exists() ) :
                    ...
                    <tr id="yith-wcwl-row-<?php echo esc_attr($item['prod_id']); ?>" data-row-id="<?php echo esc_attr($item['prod_id']); ?>">
                        ...
                        <td>
                            <div class="media">
                                <div class="product-thumbnail media-left media-middle">
                                    <a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">
                                        <?php echo trim($product->get_image()); ?>
                                    </a>
                                </div>

                                <div class="name-price media-body media-middle">
                                    ...
                                    <?php if( $show_price ) : ?>
                                        <div class="product-price">
                                            <?php echo trim($product->get_price() ? $product->get_price_html() : apply_filters( 'yith_free_text', esc_html__( 'Free!', 'greenorganic' ) )); ?>
                                        </div>
                                    <?php endif ?>
                        ...
                        <td class="product-add-to-cart">
	                        ...
	                        if( $show_dateadded && isset( $item['dateadded'] ) ):
								echo '<span class="dateadded">' . sprintf(__( 'Added on : %s', 'greenorganic' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
	                        endif;
	                        ?>
	                        ...
	                        <select class="change-wishlist selectBox">
		                        <option value=""><?php esc_html_e( 'Move', 'greenorganic' ) ?></option>
		                        <?php
		                        ...
		                        ?>
			                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
				                        ...
				                        if( $wl['wishlist_privacy'] == 1 ){
					                        $wl_privacy = esc_html__( 'Shared', 'greenorganic' );
				                        }
				                        elseif( $wl['wishlist_privacy'] == 2 ){
					                        $wl_privacy = esc_html__( 'Private', 'greenorganic' );
				                        }
				                        else{
					                        $wl_privacy = esc_html__( 'Public', 'greenorganic' );
				                        }
	                        ...
	                        <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php esc_html_e( 'Remove this product', 'greenorganic' ) ?>"><?php esc_html_e( 'Remove', 'greenorganic' ) ?></a>
                            <?php endif; ?>
                        </td>
	                    <?php endif; ?>
                        ...
                        <td class="product-remove">
                            <div>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php esc_html_e( 'Remove this product', 'greenorganic' ) ?>">&times;</a>
                            </div>
                        </td>
                        <?php endif; ?>
                    </tr>
            ...
            <tr>
                <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products were added to the wishlist', 'greenorganic' ) ) ?></td>
            </tr>
        <?php
        endif;
	        ...
	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
	            <?php if( $show_cb ) : ?>
		            <div class="custom-add-to-cart-button-cotaniner">
		                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_token ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', esc_html__( 'Add the selected products to the cart', 'greenorganic' ) ) ?></a>
		            </div>
	            <?php endif; ?>
	            ...
	            <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
		            <div class="ask-an-estimate-button-container">
	                    ...
	                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', esc_html__( 'Ask for an estimate', 'greenorganic' ) ) ?>
	                </a>
		            </div>
	            <?php endif; ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo esc_url($ask_estimate_url); ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if( ! is_user_logged_in() ): ?>
				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', esc_html__( 'Your email', 'greenorganic' ) ) ?></label>
